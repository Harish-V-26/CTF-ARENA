...
(function () {
  ...
  const authService = {
    ...
    _updateNavbar: function () {
      ...
      // Dynamically remove the Challenges option from the Learn website navbar
      ...
      // Highlight the active link dynamically based on page location on Learn website
      let homeLink = null;
      let learnLink = null;
      let rulesLink = null;
      let createRoomLink = null;
      ...
      navLinks.querySelectorAll("a").forEach(a => {
        ...
        if (text === "home") {
          ...
          rulesLink = a;
        } else if (text === "create room") {
          createRoomLink = a;
        }
        a.classList.remove("active");
      });

      if (path.includes("/challenges/") || path.includes("/learn.html")) {
        if (learnLink) learnLink.classList.add("active");
      } else if (path.includes("/rules.html")) {
        if (rulesLink) rulesLink.classList.add("active");
      } else if (path.includes("/rooms/") || path.includes("/create-room")) {
        if (createRoomLink) createRoomLink.classList.add("active");
